...
const withBroker = function* <T, E>(
  installs: ReadonlyArray<() => Operation<unknown, unknown>>,
  body: () => Operation<T, E>,
): Operation<T, unknown> {
  yield* install(BunIO)
...
const attempt = function* <T>(op: () => Operation<T, unknown>): Operation<Settled<T>, unknown> {
  try {
...
describe('CachePolicy (edge)', () => {
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          ...
          yield* Broker.actions.call(service.actions.get, ['a'])
          yield* Broker.actions.call(service.actions.get, ['a'])
          return null
        },
      ),
  ...
  it('caches an undefined return value (hit, no re-dispatch)', async () => {
    let dispatchCount = 0
    const service = buildService({
      nothing: defineAction(function* (): Operation<undefined, never> {
        dispatchCount++
        return undefined
      }),
    })
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(CachePolicy, { ttl: 60_000 })],
        function* () {
          yield* Broker.actions.register(service)
          const a = yield* Broker.actions.call(service.actions.nothing, [])
          const b = yield* Broker.actions.call(service.actions.nothing, [])
          return [a, b]
        },
      ),
    )
  ...
  it('does not cache failures — a failing call re-dispatches', async () => {
    let dispatchCount = 0
    const service = buildService({
      flaky: defineAction(function* (): Operation<string, 'transient'> {
        dispatchCount++
        if (dispatchCount === 1) {
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          yield* Broker.actions.register(service)
          const first = yield* attempt(() => Broker.actions.call(service.actions.flaky, []))
          const second = yield* attempt(() => Broker.actions.call(service.actions.flaky, []))
          return { first, second }
        },
      ),
    )
  ...
  it('concurrent identical misses do not truncate the cached TTL (orphan-timer fix)', async () => {
    ...
    const result = await run(() =>
      withBroker([() => install(service), () => install(CachePolicy, { ttl: 60 })], function* () {
        yield* Broker.actions.register(service)
        // two concurrent identical misses
        yield* all([
          Broker.actions.call(service.actions.slow, ['x']),
          Broker.actions.call(service.actions.slow, ['x']),
        ])
        // probe between the orphaned timer's deadline (10+60=70) and the real one (50+60=110)
        yield* sleep(30)
        yield* Broker.actions.call(service.actions.slow, ['x'])
        return null
      }),
    )
  ...
  it('bypasses the cache for streaming calls', async () => {
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          yield* Broker.actions.register(service)
          yield* Broker.actions.call(service.actions.get, ['a'], { streams: [] })
          yield* Broker.actions.call(service.actions.get, ['a'], { streams: [] })
          return null
        },
      ),
...
describe('RetryPolicy (edge)', () => {
  ...
  it('per-action delay override does not change the retry outcome', async () => {
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(RetryPolicy, { attempts: 5, delay: 0 })],
        function* () {
          yield* Broker.actions.register(service)
          return yield* Broker.actions.call(service.actions.flaky, [])
        },
  ...
  it('does not retry streaming calls by default (retryStreams=false)', async () => {
    let attempts = 0
    const service = buildService({
      bad: defineAction(function* (): Operation<never, 'always'> {
        attempts++
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(RetryPolicy, { attempts: 5, delay: 1 })],
        function* () {
          yield* Broker.actions.register(service)
          return yield* attempt(() => Broker.actions.call(service.actions.bad, [], { streams: [] }))
        },
      ),
    )
      ...
      withBroker(
        ...
        function* () {
          ...
          return yield* attempt(() =>
            Broker.actions.call(service.actions.flaky, [], { streams: [] }),
          )
        },
      ),
    )
...
describe('CircuitBreakerPolicy (edge)', () => {
  it('isFailure predicate: failures it rejects do not count toward the threshold', async () => {
    let attempts = 0
    const service = buildService({
      noisy: defineAction(function* (): Operation<never, 'ignore'> {
        attempts++
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          yield* Broker.actions.register(service)
          for (let i = 0; i < 5; i++) {
            yield* attempt(() => Broker.actions.call(service.actions.noisy, []))
          }
          return null
        },
      ),
  ...
  it('a success resets the consecutive-failure counter', async () => {
    ...
    const service = buildService({
      maybe: defineAction(function* (): Operation<string, 'boom'> {
        attempts++
        if (mode === 'fail') {
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          ...
          yield* attempt(() => Broker.actions.call(service.actions.maybe, []))
          yield* attempt(() => Broker.actions.call(service.actions.maybe, []))
          // a success resets the counter
          mode = 'ok'
          yield* Broker.actions.call(service.actions.maybe, [])
          // 2 more failures — still below threshold because the counter reset
          mode = 'fail'
          yield* attempt(() => Broker.actions.call(service.actions.maybe, []))
          yield* attempt(() => Broker.actions.call(service.actions.maybe, []))
          // circuit must still be closed → this call reaches the action
          return yield* attempt(() => Broker.actions.call(service.actions.maybe, []))
        },
      ),
    )
    ...
    if (isSuccess(result)) {
      expect(result.value.ok).toBe(false)
      if (!result.value.ok) {
        expect(result.value.failure.error).toBe('boom') // not CircuitOpen
      }
    }
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          yield* Broker.actions.register(service)
          for (let i = 0; i < 5; i++) {
            yield* attempt(() => Broker.actions.call(service.actions.down, []))
          }
          return null
        },
      ),
  ...
  it('keeps independent circuits for (service, action) names whose concatenation collides', async () => {
    ...
    const serviceP = namedService('ab', {
      cd: defineAction(function* (): Operation<never, 'boom'> {
        return yield* fail('boom', 'down')
      }),
    })
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          ...
          // open P.cd
          yield* attempt(() => Broker.actions.call(serviceP.actions.cd, []))
          yield* attempt(() => Broker.actions.call(serviceP.actions.cd, []))
          yield* attempt(() => Broker.actions.call(serviceP.actions.cd, []))
          // Q.d must remain independent (its own circuit, still closed)
          return yield* Broker.actions.call(serviceQ.actions.d, [])
        },
      ),
    )
  ...
  it('halfOpenMax>1: a late successful probe does not re-close a circuit re-opened by a failed probe', async () => {
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          ...
          yield* all([
            attempt(() => Broker.actions.call(service.actions.maybe, [])),
            attempt(() => Broker.actions.call(service.actions.maybe, [])),
          ])
    ...
    if (isSuccess(result)) {
      expect(result.value.ok).toBe(false)
      if (!result.value.ok) {
        expect(result.value.failure.error).toBe(CoreErrors.CircuitOpen)
      }
    }
...
describe('BulkPolicy (edge)', () => {
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          ...
          return yield* all([
            Broker.actions.call(service.actions.work, []),
            Broker.actions.call(service.actions.work, []),
          ])
        },
      ),
  ...
  it('a failing in-flight call releases its slot for the next waiter', async () => {
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(BulkPolicy, { maxConcurrent: 1, maxQueue: 5 })],
        function* () {
          yield* Broker.actions.register(service)
  ...
  it('drains the queue in FIFO submission order at maxConcurrent=1', async () => {
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(BulkPolicy, { maxConcurrent: 1, maxQueue: 10 })],
        function* () {
          yield* Broker.actions.register(service)
          return yield* all([
            Broker.actions.call(service.actions.work, [1]),
            Broker.actions.call(service.actions.work, [2]),
            Broker.actions.call(service.actions.work, [3]),
          ])
        },
      ),
    )
...
describe('TimeoutPolicy (edge)', () => {
  it('per-action disable bypasses the timeout', async () => {
    const service = buildService({
      slow: defineAction({ settings: [TimeoutPolicy.actions.disable()] }, function* (): Operation<
        string,
        never
      > {
        yield* sleep(60)
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(TimeoutPolicy, { timeoutMs: 10 })],
        function* () {
          yield* Broker.actions.register(service)
  ...
  it('times out streaming calls when timeoutStreams=true', async () => {
    const service = buildService({
      slow: defineAction(function* (): Operation<string, never> {
        yield* sleep(120)
        return 'done'
      }),
    })
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          yield* Broker.actions.register(service)
          return yield* attempt(() =>
            Broker.actions.call(service.actions.slow, [], { streams: [] }),
          )
        },
      ),
    )
    ...
    if (isSuccess(result)) {
      expect(result.value.ok).toBe(false)
      if (!result.value.ok) {
        expect(result.value.failure.error).toBe(CoreErrors.Timeout)
      }
    }
  })
  ...
  it('halts the inner work so its post-deadline side effects never run', async () => {
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(TimeoutPolicy, { timeoutMs: 20 })],
        function* () {
          yield* Broker.actions.register(service)
          const r = yield* attempt(() => Broker.actions.call(service.actions.slow, []))
...
describe('FallbackPolicy (edge)', () => {
  it('per-action config can override a global fallback value with undefined (presence-based)', async () => {
    const service = buildService({
      bad: defineAction(
        ...
        function* (): Operation<undefined, 'boom'> {
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(FallbackPolicy, { value: 'global-fallback' })],
        function* () {
          yield* Broker.actions.register(service)
  ...
  it('when() returning true applies the fallback', async () => {
    ...
    const result = await run(() =>
      withBroker([() => install(service), () => install(FallbackPolicy)], function* () {
        yield* Broker.actions.register(service)
        return yield* Broker.actions.call(service.actions.bad, [])
      }),
    )
  ...
  it('per-action disable lets the failure propagate', async () => {
    const service = buildService({
      bad: defineAction({ settings: [FallbackPolicy.actions.disable()] }, function* (): Operation<
        never,
        'boom'
      > {
        return yield* fail('boom', 'broken')
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(FallbackPolicy, { value: 'never-used' })],
        function* () {
          yield* Broker.actions.register(service)
          return yield* Broker.actions.call(service.actions.bad, [])
        },
      ...
      bad: defineAction(
        {
          settings: [
            FallbackPolicy.actions.config({
              *handler(): Operation<never, 'handler-failed'> {
...
describe('MetricsPolicy (edge)', () => {
  it('a throwing onSuccess hook does not turn a successful dispatch into a failure', async () => {
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          yield* Broker.actions.register(service)
          return yield* Broker.actions.call(service.actions.hello, [])
        },
      ),
    )
  ...
  it('per-action disable suppresses metric events', async () => {
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(MetricsPolicy, { onCall: () => calls++ })],
        function* () {
          yield* Broker.actions.register(service)
          return yield* Broker.actions.call(service.actions.hello, [])
        },
  ...
  it('records a timeout as onFailure (timeout sits inside metrics by default)', async () => {
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          yield* Broker.actions.register(service)
          return yield* attempt(() => Broker.actions.call(service.actions.slow, []))
        },
      ),
    )
  ...
  it('a timed-out half-open probe counts as a failure (timeout inside breaker) and the breaker still recovers', async () => {
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          ...
          phase = 'slow'
          yield* sleep(60)
          const timedOut = yield* attempt(() => Broker.actions.call(service.actions.maybe, []))
          // after the next reset window a fast success closes the circuit again
          phase = 'ok'
          yield* sleep(60)
          const recovered = yield* attempt(() => Broker.actions.call(service.actions.maybe, []))
    ...
    if (isSuccess(result)) {
      ...
      if (!result.value.timedOut.ok) {
        expect(result.value.timedOut.failure.error).toBe(CoreErrors.Timeout)
      }
  ...
  it('fallback serves a value after retry exhausts its attempts', async () => {
    ...
    const service = buildService({
      bad: defineAction(
        ...
        function* (): Operation<string, 'always'> {
          attempts++
          return yield* fail('always', 'nope')
        },
      ),
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          ...
          const a = yield* Broker.actions.call(service.actions.down, [])
          const b = yield* Broker.actions.call(service.actions.down, [])
          // now the breaker is open → CircuitOpen, which fallback also catches
          const c = yield* Broker.actions.call(service.actions.down, [])
          return [a, b, c]
        },
      ),
    )
...
describe('core wiring (edge)', () => {
  it('a per-action setting for an uninstalled policy does not break other policies', async () => {
    ...
    const result = await run(() =>
      withBroker([() => install(service), () => install(CachePolicy, { ttl: 60_000 })], function* () {
        yield* Broker.actions.register(service)
        const a = yield* Broker.actions.call(service.actions.get, [])
        const b = yield* Broker.actions.call(service.actions.get, [])
        return [a, b]
      }),
    )
  ...
  it('does not coalesce calls whose service+action names would collide without a key separator', async () => {
    const serviceP = namedService('ab', {
      cd: defineAction(function* (): Operation<string, never> {
        yield* sleep(30)
        return 'from-P'
      }),
    })
    const serviceQ = namedService('abc', {
      d: defineAction(function* (): Operation<string, never> {
        yield* sleep(30)
        return 'from-Q'
      }),
    })
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          ...
          return yield* all([
            Broker.actions.call(serviceP.actions.cd, []),
            Broker.actions.call(serviceQ.actions.d, []),
          ])
        },
      ),
    )
