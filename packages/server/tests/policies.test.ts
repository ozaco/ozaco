...
describe('BucketPolicy', () => {
  it('coalesces identical in-flight non-stream requests', async () => {
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(BucketPolicy, { interval: 100, max: 100 })],
        function* () {
          yield* Broker.actions.register(service)
          return yield* all([
            Broker.actions.call(service.actions.slow, [21]),
            Broker.actions.call(service.actions.slow, [21]),
            Broker.actions.call(service.actions.slow, [21]),
            Broker.actions.call(service.actions.slow, [21]),
            Broker.actions.call(service.actions.slow, [21]),
  ...
  it('does NOT coalesce when max is reached — new bucket opens', async () => {
    let dispatchCount = 0
    const service = buildService({
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(BucketPolicy, { interval: 100, max: 2 })],
        function* () {
          yield* Broker.actions.register(service)
          return yield* all([
            Broker.actions.call(service.actions.slow, []),
            Broker.actions.call(service.actions.slow, []),
            Broker.actions.call(service.actions.slow, []),
            Broker.actions.call(service.actions.slow, []),
          ])
    ...
    expect(isSuccess(result)).toBe(true)
    // max=2 — first bucket holds 2 calls, next 2 open a fresh bucket → 2 dispatches
  ...
  it('different params do not coalesce', async () => {
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(BucketPolicy, { interval: 100 })],
        function* () {
          yield* Broker.actions.register(service)
          return yield* all([
            Broker.actions.call(service.actions.slow, [1]),
            Broker.actions.call(service.actions.slow, [2]),
            Broker.actions.call(service.actions.slow, [3]),
          ])
        },
      ),
    )
  ...
  it('per-action disable bypasses coalescing', async () => {
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(BucketPolicy, { interval: 1000, max: 100 })],
        function* () {
          yield* Broker.actions.register(service)
          return yield* all([
            Broker.actions.call(service.actions.uncoalesced, [1]),
            Broker.actions.call(service.actions.uncoalesced, [1]),
            Broker.actions.call(service.actions.uncoalesced, [1]),
          ])
        },
  ...
  it('stream calls pass through untouched', async () => {
    ...
    const result = await run(() =>
      withBroker([() => install(service), () => install(BucketPolicy)], function* () {
        yield* Broker.actions.register(service)
        return yield* Broker.actions.call(service.actions.ping, [7])
      }),
    )
...
describe('RetryPolicy', () => {
  ...
  it('attempts=1 means no retry', async () => {
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(RetryPolicy, { attempts: 1 })],
        function* () {
          yield* Broker.actions.register(service)
  ...
  it('exhausts attempts then surfaces final failure', async () => {
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(RetryPolicy, { attempts: 3, delay: 1 })],
        function* () {
          yield* Broker.actions.register(service)
          return yield* Broker.actions.call(service.actions.bad, [])
        },
  ...
  it('per-action attempts override raises retry budget', async () => {
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(RetryPolicy, { attempts: 2, delay: 1 })],
        function* () {
          yield* Broker.actions.register(service)
          return yield* Broker.actions.call(service.actions.flaky, [])
        },
  ...
  it('per-action disable skips retry entirely', async () => {
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(RetryPolicy, { attempts: 10, delay: 1 })],
        function* () {
          yield* Broker.actions.register(service)
          return yield* Broker.actions.call(service.actions.bad, [])
        },
...
describe('CachePolicy', () => {
  it('serves identical calls from cache within TTL', async () => {
    ...
    const result = await run(() =>
      withBroker([() => install(service), () => install(CachePolicy, { ttl: 1000 })], function* () {
        yield* Broker.actions.register(service)
        const a = yield* Broker.actions.call(service.actions.lookup, ['x'])
        const b = yield* Broker.actions.call(service.actions.lookup, ['x'])
        const c = yield* Broker.actions.call(service.actions.lookup, ['y'])
        return [a, b, c]
      }),
    )
  ...
  it('re-dispatches after TTL expires', async () => {
    ...
    const result = await run(() =>
      withBroker([() => install(service), () => install(CachePolicy, { ttl: 30 })], function* () {
        yield* Broker.actions.register(service)
        const a = yield* Broker.actions.call(service.actions.now, [])
        yield* sleep(60)
        const b = yield* Broker.actions.call(service.actions.now, [])
        return [a, b]
      }),
    )
  ...
  it('evicts oldest entry when max is reached (FIFO)', async () => {
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(CachePolicy, { ttl: 60_000, max: 2 })],
        function* () {
          yield* Broker.actions.register(service)
          yield* Broker.actions.call(service.actions.lookup, ['a']) // d=1; cache: [a]
          yield* Broker.actions.call(service.actions.lookup, ['b']) // d=2; cache: [a,b]
          yield* Broker.actions.call(service.actions.lookup, ['c']) // d=3; evict a; cache: [b,c]
          yield* Broker.actions.call(service.actions.lookup, ['b']) // hit
          yield* Broker.actions.call(service.actions.lookup, ['a']) // d=4 (was evicted)
          return null
        },
      ),
    )
  ...
  it('per-action ttl override beats global', async () => {
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          yield* Broker.actions.register(service)
          const a = yield* Broker.actions.call(service.actions.hot, [])
          const b = yield* Broker.actions.call(service.actions.hot, [])
          yield* sleep(60)
          const c = yield* Broker.actions.call(service.actions.hot, [])
          return [a, b, c]
        },
      ),
    )
  ...
  it('per-action disable bypasses cache', async () => {
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          yield* Broker.actions.register(service)
          const a = yield* Broker.actions.call(service.actions.uncacheable, [])
          const b = yield* Broker.actions.call(service.actions.uncacheable, [])
          return [a, b]
        },
      ),
    )
...
describe('CircuitBreakerPolicy', () => {
  it('opens after threshold and fast-fails subsequent calls', async () => {
    ...
    if (isFailure(circuitOpenError)) {
      expect(circuitOpenError.error).toBe(CoreErrors.CircuitOpen)
    }
  })
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          ...
          succeed = true
          const value = yield* Broker.actions.call(service.actions.maybe, [])

          // circuit should be closed now — many more successes pass
          const value2 = yield* Broker.actions.call(service.actions.maybe, [])
          return [value, value2]
        },
      ),
    )
  ...
  it('half-open: failure transitions back to open', async () => {
    ...
    if (isFailure(secondOpenError)) {
      expect(secondOpenError.error).toBe(CoreErrors.CircuitOpen)
    }
  })
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          ...
          for (let i = 0; i < 2; i++) {
            try {
              yield* Broker.actions.call(service.actions.sensitive, [])
            } catch {
              // expected
            }
          }
...
describe('BulkPolicy', () => {
  it('limits concurrency at maxConcurrent', async () => {
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(BulkPolicy, { maxConcurrent: 2, maxQueue: 10 })],
        function* () {
          yield* Broker.actions.register(service)
          return yield* all([
            Broker.actions.call(service.actions.slow, []),
  ...
  it('rejects with BulkQueueFull when queue is exhausted', async () => {
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(BulkPolicy, { maxConcurrent: 1, maxQueue: 1 })],
        function* () {
          yield* Broker.actions.register(service)
          try {
            yield* all([
  ...
  it('per-action disable bypasses bulkhead', async () => {
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(BulkPolicy, { maxConcurrent: 1 })],
        function* () {
          yield* Broker.actions.register(service)
          return yield* all([
            Broker.actions.call(service.actions.uncapped, []),
            Broker.actions.call(service.actions.uncapped, []),
            Broker.actions.call(service.actions.uncapped, []),
          ])
        },
...
describe('TimeoutPolicy', () => {
  it('fails with Timeout when dispatch exceeds deadline', async () => {
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(TimeoutPolicy, { timeoutMs: 30 })],
        function* () {
          yield* Broker.actions.register(service)
    ...
    expect(isFailure(result)).toBe(true)
    if (isFailure(result)) {
      expect(result.error).toBe(CoreErrors.Timeout)
    }
  })
  ...
  it('passes through under deadline', async () => {
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(TimeoutPolicy, { timeoutMs: 500 })],
        function* () {
          yield* Broker.actions.register(service)
          return yield* Broker.actions.call(service.actions.fast, [])
        },
      ),
    )
  ...
  it('per-action timeoutMs override', async () => {
    ...
    const result = await run(() =>
      withBroker(
        [() => install(service), () => install(TimeoutPolicy, { timeoutMs: 10_000 })],
        function* () {
          yield* Broker.actions.register(service)
          return yield* Broker.actions.call(service.actions.slow, [])
        },
  ...
  it('stream calls pass through without timeout', async () => {
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          yield* Broker.actions.register(service)
          return yield* Broker.actions.call(service.actions.ping, [9])
        },
      ),
    )
...
describe('FallbackPolicy', () => {
  ...
  it('passes through on success', async () => {
    ...
    const result = await run(() =>
      withBroker([() => install(service), () => install(FallbackPolicy)], function* () {
        yield* Broker.actions.register(service)
        return yield* Broker.actions.call(service.actions.good, [])
      }),
    )
  ...
  it('emits onCall + onSuccess on success with positive durationMs', async () => {
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          yield* Broker.actions.register(service)
          return yield* Broker.actions.call(service.actions.hello, ['world'])
        },
      ),
    )
  ...
  it('cache short-circuits the chain — retry/cb not invoked on hit', async () => {
    ...
    const result = await run(() =>
      withBroker(
        ...
        function* () {
          yield* Broker.actions.register(service)
          yield* Broker.actions.call(service.actions.lookup, ['a'])
          yield* Broker.actions.call(service.actions.lookup, ['a'])
          yield* Broker.actions.call(service.actions.lookup, ['a'])
          return null
        },
