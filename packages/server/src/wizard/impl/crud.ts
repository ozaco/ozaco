...
export const buildCrudModule = <T extends TableDef>(
  ...
  const resolveId = function* (db: AnyType, body: AnyType) {
  ...
  const assertIfMatch = function* (db: AnyType, id: string) {
  ...
  const assertVersion = function* (db: AnyType, id: string, expected: string | undefined) {
