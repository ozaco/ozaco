#!/usr/bin/env bash
# Run the carrier suite against NATS (JetStream) and Redis. Uses TRANSPORT_TEST_NATS_URL /
# TRANSPORT_TEST_REDIS_URL when already set; otherwise spins disposable containers.
set -euo pipefail
cd "$(dirname "$0")/.."

if [ -n "${TRANSPORT_TEST_NATS_URL:-}" ] && [ -n "${TRANSPORT_TEST_REDIS_URL:-}" ]; then
  exec bun test tests/carrier "$@"
fi
