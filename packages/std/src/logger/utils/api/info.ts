...
export const createInfo = (logger: Std.Logger.Api) => {
  const mark = picocolors.bgBlue(' info ')

  const noConsole = logger.options.levelIndex > LOGGER_LEVELS.indexOf('info')
  ...
  return capsule(handler, loggerTags.get('info'))
}
