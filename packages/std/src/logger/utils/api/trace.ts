...
export const createTrace = (logger: Std.Logger.Api) => {
  const mark = picocolors.italic(' trace ')

  const noConsole = logger.options.levelIndex > LOGGER_LEVELS.indexOf('trace')
  ...
  return capsule(handler, loggerTags.get('trace'))
}
