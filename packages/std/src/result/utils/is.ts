...
export const isSuccess = <T>(value: unknown): value is Success<T> => {
  return typeof value === 'object' && value !== null && '_t' in value && value._t === RESULT_SUCCESS
}

export const isFailure = <E>(value: unknown): value is Failure<E> => {
  return typeof value === 'object' && value !== null && '_t' in value && value._t === RESULT_FAILURE
}
...
export const isJust = <T>(value: unknown): value is Just<T> => {
  return typeof value === 'object' && value !== null && '_t' in value && value._t === MAYBE_JUST
}

export const isNothing = <T>(value: unknown): value is Nothing<T> => {
  return typeof value === 'object' && value !== null && '_t' in value && value._t === MAYBE_NOTHING
}
