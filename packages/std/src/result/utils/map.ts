...
import type { Impl, InferFailure, Result, ResultFor, ResultMaybeAsync } from '../types'

import { auto } from './auto'
import { isFailure } from './is'

export const map: Impl.Map = <T1, T2>(fn: (a: T1) => T2) => {
  return <R1 extends ResultMaybeAsync<T1, BlobType>>(result: R1): ResultFor<R1, T2, InferFailure<R1>> => {
