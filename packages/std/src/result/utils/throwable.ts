  ...
  try {
    ...
    if (isPromise(result)) {
      return result.then(auto as BlobType, (err: BlobType) => {
        ...
        return fail(new CustomError(err as BlobType), 'from throwable', ...causes)
      })
    }
    ...
    if (err instanceof CustomError) return fail(err, 'from throwable', ...causes) as BlobType

    return fail(new CustomError(err as BlobType), 'from throwable', ...causes) as BlobType
  }
}
