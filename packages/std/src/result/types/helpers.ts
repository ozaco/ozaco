...
export namespace Helpers {
  export type InferSuccess<T> = [T] extends [
    ...
    ? U
    : [T] extends [ResultMaybeAsync<infer U, BlobType>]
      ? U
      : never
  ...
  export type InferFailure<T> = [T] extends [
    ...
    ? U
    : [T] extends [ResultMaybeAsync<BlobType, infer U>]
      ? U
      : never

  export type UnionToResult<R> =
    R extends Result<BlobType, BlobType> ? R : ResultFor<false, R, never>
