...
export const createPlugin = capsule(
  <const M extends Std.Plugin.Meta<BlobType, BlobType, BlobType>>(meta: M): Std.Plugin.BasePlugin<M> => {
        ...
        for (const action of actions) {
          ...
          Object.assign(ctx, instance, {
            ...
            $peek: $fn(targetAction => {
              ...
              return err(
                `${meta.name}@${meta.version}#${targetAction.$name}/not-registered`,
                `action (${targetAction.$name}) not registered`,
              )
            ...
            'apply' | 'tag' | '$peek' | '$get' | '$fn' | '$safe' | '$capsule' | '$signal'
          >)

          const actionResult = action(ctx)

          const applyActionResult = (
            resultCtx: Std.Plugin.Actions.Context<BlobType, BlobType, BlobType, BlobType, BlobType>,
          ) => {
