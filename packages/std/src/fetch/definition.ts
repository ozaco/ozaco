 ...
 * `yield* res.expect()` first). Every verb shorthand delegates to the PINNED
 * `FetchClient.actions.request`, so `Fetch.around({ request })` (and `before`/`after`/`error`)
...
/** A verb shorthand: pins `method` and delegates to the PINNED `FetchClient.actions.request` —
 * hooks installed against `request` still wrap the call. */
...
/**
 * The platform-fetch implementation of the protocol: `FetchClient.use({ baseUrl, headers,
