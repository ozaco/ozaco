...
/** Run `body` atomically on one reserved connection; nested calls become savepoints. */
export const runTransaction = operation(function* (body: () => AnyType) {
  ...
  const reserve = yield* attempt(until(state.client.reserve() as Promise<AnyType>))
  if (isFailure(reserve)) {
    return yield* raise(reserve.error)
  }
  const session = reserve.value
  try {
    return yield* TxSession.with(session, () =>
