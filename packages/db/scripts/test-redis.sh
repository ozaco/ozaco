#!/usr/bin/env bash
# Run the Kv suite against REDIS. Uses TRANSPORT_TEST_REDIS_URL when already set; otherwise spins
# a disposable redis:7-alpine container (random host port) and tears it down after.
set -euo pipefail
cd "$(dirname "$0")/.."

if [ -n "${TRANSPORT_TEST_REDIS_URL:-}" ]; then
  exec bun test tests/kv/redis.test.ts "$@"
fi
...
TRANSPORT_TEST_REDIS_URL="redis://127.0.0.1:${PORT}" bun test tests/kv/redis.test.ts "$@"
