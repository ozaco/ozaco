  ...
  describe.skipIf(!target.enabled)(`e2e — ${target.label}`, () => {
    ...
    it('keyset pagination forward and backward', async () => {
      await runScoped(function* () {
        ...
        const pageTwo = yield* query().paginate({ limit: 2, cursor: pageOne.pageInfo.nextCursor })
        expect(pageTwo.data.map((row: AnyType) => row.name)).toEqual(['c', 'd'])

        const pageThree = yield* query().paginate({ limit: 2, cursor: pageTwo.pageInfo.nextCursor })
    ...
    it('watch: delta mode, coalescing, query-aware skip and since resume', async () => {
      await runScoped(function* () {
        ...
        expect((changed.value as AnyType).changed.map((row: AnyType) => row.name)).toEqual(['ada'])
